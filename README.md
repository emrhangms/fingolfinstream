...
3. Ana dala (`main` veya `master`) kod gönderdiğinizde GitHub Actions eklentiyi otomatik olarak derler ve `builds` dalına yükler.
4. Cloudstream uygulamasında **Ayarlar > Eklentiler > Depo Ekle** kısmına deponuzun bağlantısını girin:
   ```text
   https://raw.githubusercontent.com/emrhangms/fingolfinstream/builds/plugins.json
   ```
